...
for role in ('assault','ling','chain'):
    ...
    p.write_text(tmpl)
# update evals with richer assertions
ep = pathlib.Path('evals/evals.json')
data = json.loads(ep.read_text(encoding='utf-8'))
for ex in data.get('evals', []):
    if ex.get('id') == 1:
        ex['assertions'] = [
            {"type": "contains", "text": "status"},
            {"type": "contains", "text": "engine"},
        ]
    elif ex.get('id') == 2:
        ex['assertions'] = [
            {"type": "contains", "text": "ling-1"},
            {"type": "contains", "text": "engine"},
        ]
...
if not readme.exists():
    readme.write_text(textwrap.dedent('''
        # MinisSkills
        Individual skill modules under `MinisSkills/`.
        Each module contains `SKILL.md` and `scripts/run.py`.
        Use `scripts/run.py` to invoke the skill's `run(payload)`.
        ## Run example
        ```bash
